...
public:
	UnderwaterChannelClass() :
			TclClass("Channel/UnderwaterChannel") {
	}
...
UnderwaterChannel::UnderwaterChannel(void) :
		Channel(), numNodes_(0), xListHead_(NULL), sorted_(0) {
}
...
void UnderwaterChannel::sendUp(Packet* p, Phy *tifp) {
...
double UnderwaterChannel::distance(Node *sender, Node* receiver) {
	MobileNode *s1, *r1;
...
void UnderwaterChannel::calculatePosition(Node *sender, Node* receiver,
		Packet* p) {
	MobileNode *s1, *r1;
...
void UnderwaterChannel::updateNodesList(class MobileNode *mn, double oldX) {
	...
	if (mn->nextX_ != NULL) {
		if (mn->prevX_ != NULL) {
			if ((mn->nextX_->X() >= X) && (mn->prevX_->X() <= X))
				skipX = true; // the node doesn't change its position in the list
			else {
			...
			if (mn->nextX_->X() >= X)
				skipX = true; // skip updating the first element
			else {
		...
		if (mn->prevX_->X() <= X)
			skipX = true; // skip updating the last element
		else
			mn->prevX_->nextX_ = NULL;
	}
	...
	if (!skipX) {
		if (X > oldX) {
			for (tmp = mn; tmp->nextX_ != NULL && tmp->nextX_->X() < X; tmp =
					tmp->nextX_)
				;
			...
			for (tmp = mn; tmp->prevX_ != NULL && tmp->prevX_->X() > X; tmp =
					tmp->prevX_)
				;
